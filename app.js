...
setMode('mobile'); // Always start in mobile mode
renderCalendar();
renderEventList(); // Initial render for list view
...
// Swipe Navigation for Calendar
// calendarSection is already declared at the top
if (calendarSection) {
    if (calendarSection) {
        let touchStartX = 0;
        let touchEndX = 0;
    ...
    function saveEvents() {
        ...
        localStorage.setItem('calendar_deleted_events', JSON.stringify(deletedEvents));
        renderCalendar();
        renderEventList(); // Update list view on save
    }
...
if (tabCalendar && tabList) {
    ...
    tabList.addEventListener('click', () => {
        ...
        renderEventList(); // Refresh list when switching
    });
}
...
function renderEventList() {
    ...
    const sortedKeys = Object.keys(grouped).sort((a, b) => {
        return grouped[a][0].start - grouped[b][0].start;
    });
    ...
    sortedKeys.forEach(dateStr => {
        ...
        // Extract Month/Day for display (e.g. "11월 27일")
        const displayDate = dateStr.replace(/^\d{4}년\s/, '');
        ...
        grouped[dateStr].forEach(e => {
            ...
            if (e.isAllDay) {
                ...
                timeEl.textContent = e.start.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false });
            }
